...
pub async fn check_storage_quota(
    ...
    let client = match state.db.get().await {
        Ok(client) => client,
        Err(e) => {
            tracing::error!("Error getting db client: {}", e);
            return AppError::Internal("Failed to check storage quota".to_string()).into_response();
        }
    };

    let storage_info = repositories::user::get_user_storage_info(&client, &user_id, &state.stmt_cache).await;
...
pub async fn verify_file_ownership(
    ...
    let client = match state.db.get().await {
        ...
        Err(e) => {
            tracing::error!("Error getting db client: {}", e);
            return AppError::Internal("Failed to verify file ownership".to_string()).into_response();
        }
    };

    let file_owner = repositories::file::find_by_id(&client, file_id, user_id, &state.stmt_cache).await;
